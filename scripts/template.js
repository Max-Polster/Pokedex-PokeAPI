function pokemonCardTemplate(index, data) {
  return `
      ...
      } rounded-4 shadow-sm text-white" role="button" onclick="openInfo(${index})" data-pokemon-id="${
    data["id"]
  }">
        <div class="card-body fw-medium d-flex flex-column">
          <div class="number d-flex justify-content-end text-white text-opacity-50">#${String(
            data["id"]
          ).padStart(3, 0)}</div>
...
function pokemonBigCardTemplate(index, data) {
  return `
      <div class="card pokemon-card big ${
        data["types"][0]["type"]["name"]
      } rounded-5 shadow-sm flex-fill z-3 h-100" onclick="event.stopPropagation();">
        <div class="card-header border-0 bg-transparent text-white p-3 py-5 p-sm-5">
          ...
          <div class="d-flex justify-content-between align-items-end">
            ...
            <div class="number text-white fw-medium fs-5">#${String(
              data["id"]
            ).padStart(3, 0)}</div>
          </div>
        ...
        <div class="card-body bg-white rounded-5 fw-medium d-flex flex-column p-3 py-5 p-sm-5 overflow-hidden">
          ...
          ${pokemonBigCardBaseAboutTemplate(
            data["id"],
            data["height"],
            data["weight"],
            data["name"],
